...
data2=( `cat /var/log/xray/trojan.log | grep $akun | tail -n 100 | cut -d " " -f 3 | cut -d ":" -f 1 | sort | uniq`);
echo -n > /tmp/iptrojan.txt
for ip in "${data2[@]}"
do
jum=$(cat /var/log/xray/trojan.log | grep -w $akun | awk '{print $3}' | tail -n 1000 | cut -d " " -f 3 | cut -d ":" -f 1 | grep -w $ip | sort | uniq | sed '/tcp/d' | sed '/udp/d')
if [[ -z "$jum" ]]; then
echo > /dev/null
else
