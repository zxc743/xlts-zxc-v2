...
#Trojan
data=( `cat /etc/xray/trojan.json | grep '^###' | cut -d ' ' -f 2 | cut -d '-' -f 2`);
now=`date +"%Y-%m-%d"`
for user in "${data[@]}"
do
...
#vless
data=( `cat /etc/xray/vless.json | grep '^###' | cut -d ' ' -f 2 | cut -d '-' -f 2`);
now=`date +"%Y-%m-%d"`
for user in "${data[@]}"
do
...
#vlgrpc
data=( `cat /etc/xray/vlgrpc.json | grep '^###' | cut -d ' ' -f 2 | cut -d '-' -f 2`);
now=`date +"%Y-%m-%d"`
for user in "${data[@]}"
do
exp=$(grep -w "^### $user" "/etc/xray/vlgrpc.json" | cut -d ' ' -f 3)
d1=$(date -d "$exp" +%s)
d2=$(date -d "$now" +%s)
exp2=$(( (d1 - d2) / 86400 ))
if [[ "$exp2" = "0" ]]; then
...
#Trojanws
data=( `cat /etc/xray/trojanws.json | grep '^###' | cut -d ' ' -f 2 | cut -d '-' -f 2`);
now=`date +"%Y-%m-%d"`
for user in "${data[@]}"
do
exp=$(grep -w "^### $user" "/etc/xray/trojanws.json" | cut -d ' ' -f 3)
d1=$(date -d "$exp" +%s)
d2=$(date -d "$now" +%s)
exp2=$(( (d1 - d2) / 86400 ))
if [[ "$exp2" = "0" ]]; then
